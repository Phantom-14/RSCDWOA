function drawPc(x,option,data,str)
x0=x;
pic_num=1;
for i=1:data.numUAV
...
while 1
    ...
    for noUAV=1:data.numUAV
        %% smooth
        Y=[];
        for i=1:length(path{noUAV}(1,:))
            temp=path{noUAV}(:,i);
            X0=1:length(temp);
            X=X0(1):1/length(temp):X0(end);
            Y(:,i)=interp1(X0,temp,X,"spline");
        end
       %   plot3(path{noUAV}(:,1),path{noUAV}(:,2),path{noUAV}(:,4),'-','LineWidth',2)
        plot3(Y(:,1),Y(:,2),Y(:,4),'-','LineWidth',2)
        hold on

    end

    %mesh(map'+10*myMapminmax(Rep_M))
    mesh(map')
    %mesh(F)
    if ~isempty(data.Ob_M)
        for i=1:length(data.Ob_M(:,1))
    ...
    %title('potential force') the Resultant Force of Potential Field
    xlabel('x/km')
    ylabel('y/km')
    zlabel('z/km')
    ...
    Fa = getframe(gcf);  
    Im = frame2im(Fa);   
    [A, maap] = rgb2ind(Im, 256); 
    figname=cat(2,str,'.gif');
    if pic_num == 1
        imwrite(A, maap, figname, 'gif', 'Loopcount', Inf, 'DelayTime', 0.1);
        
    else
        imwrite(A, maap,figname,'gif','WriteMode','append','DelayTime',0.1);
